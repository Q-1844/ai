...
### 修正：视频最终出片成功（2026-06-12 二次实测）

原先以为"视频任务永远 queued"的结论**不准确**。原因是查询接口用错了：

- **正确接口**：`GET /agnesapi?video_id=<video_id>&model_name=agnes-video-v2.0`（官方推荐，5s 轮询）
- **错误接口**：`GET /v1/videos/{task_id}`（历史兼容，状态**严重延迟**，会一直显示 `queued`）

修正接口后，9 个任务中的 **6 个已 completed**，下载到本地 MP4 文件大小 562 KB – 1.6 MB，正常 5 秒视频。
剩余 3 个仍在队列中，可能需要更长时间或额外的资源调度。

**给工程实现的关键教训**：
1. 始终用创建时返回的 `video_id`（不是 `task_id`）去查结果
2. 轮询间隔按文档建议 = 5 秒
3. 不要因为 `task_id` 接口长期 `queued` 就认为任务失败——它可能早完成了
4. 完整对比表见 [AGNES_MODELS.md](./AGNES_MODELS.md) 顶部"⚠️ 视频查询接口踩坑"一节
